...
library(gganimate)
library(lubridate)
SFWatch <- read_csv("./Data/SelectedFoodWatch.csv")

SFWatch_tidy  <- SFWatch %>% filter(State == "Lagos", Classification == "Food Item") 
...
anim2 <- ggplot(SFWatch_formatted, aes(rank, group = GenItemLabel, 
                                  fill = as.factor(GenItemLabel), color = as.factor(GenItemLabel))) +
  ...
  theme(axis.line=element_blank(),
        ...
        plot.title=element_text(size=20, hjust=0.5,face="bold", vjust=-1),
  ...
  labs(title = 'Price in {closest_state}',  
...
animate(anim2, nframes = 300, fps = 5, width = 600, height = 400, 
